...
void CG_DrawForceSelect( void )
{
	int		i;
	int		count;
	float	smallIconSize_x,smallIconSize_y,bigIconSize_x,bigIconSize_y;
	float	holdX,pad;
	int		x,y;
	...
	smallIconSize_x = 30 * cgs.widthRatioCoef, smallIconSize_y = 30;
	bigIconSize_x = 60 * cgs.widthRatioCoef, bigIconSize_y = 60;
	pad = 12;
	...
	for (iconCnt=1;iconCnt<(sideLeftIconCnt+1);i--)
	{
		...
		{
			CG_DrawPic( holdX, y + yOffset, smallIconSize_x, smallIconSize_y, cgs.media.forcePowerIcons[forcePowerSorted[i]] );
	...
	if (ForcePower_Valid(cg.forceSelect))
	{
		...
		{
			CG_DrawPic( x-(bigIconSize_x/2), (y-((bigIconSize_y-smallIconSize_y)/2)) + yOffset, bigIconSize_x, bigIconSize_y, cgs.media.forcePowerIcons[cg.forceSelect] ); //only cache the icon for display
		}
	}
	...
	for (iconCnt=1;iconCnt<(sideRightIconCnt+1);i++)
	{
		...
		if (cgs.media.forcePowerIcons[forcePowerSorted[i]])
		{
			CG_DrawPic( holdX, y + yOffset, smallIconSize_x, smallIconSize_y, cgs.media.forcePowerIcons[forcePowerSorted[i]] ); //only cache the icon for display
			holdX += (smallIconSize_x+pad);
		}
	}
...
//const int bits = cg.snap->ps.stats[ STAT_ITEMS ];
	...
	smallIconSize_x = 40 * cgs.widthRatioCoef, smallIconSize_y = 40;
	bigIconSize_x = 80 * cgs.widthRatioCoef, bigIconSize_y = 80;
	pad = 16 * cgs.widthRatioCoef;
...
//	addX = (float) smallIconSize * .75;
	...
	for (iconCnt=0;iconCnt<sideLeftIconCnt;i--)
	{
		...
		{
			trap->R_SetColor(NULL);
			CG_DrawPic( holdX, y+10, smallIconSize_x, smallIconSize_y, cgs.media.invenIcons[i] );
	...
	if (cgs.media.invenIcons[cg.itemSelect] && BG_IsItemSelectable(&cg.predictedPlayerState, cg.itemSelect))
	{
		int itemNdex;
		trap->R_SetColor(NULL);
		CG_DrawPic( x-(bigIconSize_x/2), (y-((bigIconSize_y-smallIconSize_y)/2))+10, bigIconSize_x, bigIconSize_y, cgs.media.invenIcons[cg.itemSelect] );
...
void CG_DrawVehicleDamage(const centity_t *veh,int brokenLimbs,const menuDef_t	*menuHUD,float alpha,int index,int isEnemyVehicleHUD)
{
	...
	if (item)
	{
		...
		{
			CG_DrawPic(
				isEnemyVehicleHUD ? SCREEN_WIDTH - (SCREEN_WIDTH - item->window.rect.x) * cgs.widthRatioCoef : item->window.rect.x * cgs.widthRatioCoef,
				item->window.rect.y,
...
void CG_DrawVehicleDamageHUD(const centity_t *veh,int brokenLimbs,float percShields,char *menuName, float alpha)
{
	...
	CG_DrawVehicleDamage(veh,brokenLimbs,menuHUD,alpha,VEH_DAMAGE_FRONT,isEnemyVehicleHUD);
	CG_DrawVehicleDamage(veh,brokenLimbs,menuHUD,alpha,VEH_DAMAGE_BACK,isEnemyVehicleHUD);
	CG_DrawVehicleDamage(veh,brokenLimbs,menuHUD,alpha,VEH_DAMAGE_LEFT,isEnemyVehicleHUD);
	CG_DrawVehicleDamage(veh,brokenLimbs,menuHUD,alpha,VEH_DAMAGE_RIGHT,isEnemyVehicleHUD);
}
...
void CG_DrawFlagStatus()
{
	...
	if (CG_YourTeamHasFlag())
	{
		...
		CG_DrawPic( 2, 330-startDrawPos, ico_size * cgs.widthRatioCoef, ico_size, theirFlagShader );
	...
	if (CG_OtherTeamHasFlag())
	{
		...
		CG_DrawPic( 2, 330-startDrawPos, ico_size * cgs.widthRatioCoef, ico_size, myFlagTakenShader );
	}
}
